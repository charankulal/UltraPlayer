...
public class VideoFilesActivity extends AppCompatActivity implements SearchView.OnQueryTextListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        swipeRefreshLayout=findViewById(R.id.swipe_to_refresh_files);

        SharedPreferences.Editor editor=getSharedPreferences(MY_PREF,MODE_PRIVATE).edit();
        editor.putString("playlistName",folder_name);
        editor.apply();
    ...
    public boolean onOptionsItemSelected(@NonNull MenuItem item) {
        ...
        if(id==R.id.sort_by)
        {
            ...
            String[] items={"Name (A-Z)","Size (Large-Small)","Date (New - Old)","Length (Long - Short)"};
    ...
    public boolean onQueryTextChange(String newText) {
        ...
        for (MediaFiles mediaFiles1:videoFilesArrayList)
        {
            if(mediaFiles1.getTitle().toLowerCase().contains(inputs))
            {
                mediaFiles.add(mediaFiles1);
            }
        }
